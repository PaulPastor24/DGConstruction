<?php $__env->startSection('title', 'Client Timeline - D&G Construction Monitor'); ?>
<?php $__env->startSection('page_title', 'Project Timeline'); ?>
<?php $__env->startSection('mobileTitle', 'Timeline'); ?>
...
<style>
    ...
    @media (max-width: 800px) {
        ...
        #pg-timeline .top-toolbar > .toolbar-group.search-group,
        #pg-timeline .top-toolbar > .toolbar-group:not(:has(#phaseFilterSelector)):not(:has(#statusFilterSelector)) {
            width: 100% !important;
            min-width: 0 !important;
        ...
        #pg-timeline .top-toolbar > .toolbar-group:has(#phaseFilterSelector),
        #pg-timeline .top-toolbar > .toolbar-group:has(#statusFilterSelector) {
    ...
    @media (max-width: 1100px) {
        ...
        #pg-timeline .gantt_hor_scroll,
        #pg-timeline .scrollHor_cell {
        ...
        #pg-timeline .gantt_hor_scroll::-webkit-scrollbar,
        #pg-timeline .scrollHor_cell::-webkit-scrollbar {
            width: 0 !important;
            height: 0 !important;
            display: none !important;
        }
...
<?php $__env->startPush('scripts'); ?>
    <?php echo app('Illuminate\Foundation\Vite')(['resources/js/admin-timeline.js']); ?>
<?php $__env->stopPush(); ?>
    ...
    <div class="top-toolbar">
        ...
        <div class="toolbar-group">
            ...
            <select id="projectSelector" class="toolbar-select">
                <?php $__currentLoopData = $projectsWithStats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $project): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<script>
    const projectsData = <?php echo json_encode($projectsWithStats, 15, 512) ?>;
</script>
...
<script>
    ...
    function selectProject(projectId) {
        const preferredProject = projectsData.find((project) => String(project.id) === String(projectId))
            || projectsData.find((project) => Array.isArray(project.phases) && project.phases.length > 0)
            || projectsData[0]
            || null;
                                    ...
                                    <tbody>
                                        ${paginatedMilestones.length ? paginatedMilestones.map((milestone, index) => {
                                            ...
                                            return `
                                             ...
                                             `;
                                         }).join('') : `<tr><td colspan="6"><div class="timeline-empty-state">No milestones match the current filters.</div></td></tr>`}
                                    </tbody>
                                </table>
                            <div class="timeline-table-footer">
    ...
    async function refreshTimelineFromServer() {
        ...
        try {
            const response = await fetch(getClientUrl(`/client/timeline/data/${selectedProject.id}`), {
                headers: {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (selector && selector.options.length > 0) {
            const projectOptions = Array.from(selector.options).map((option) => option.value);
            const fallbackProjectId = projectOptions.find((value) => {
                const project = projectsData.find((item) => String(item.id) === String(value));
                return !!project && Array.isArray(project.phases) && project.phases.length > 0;
            }) || projectOptions[0];

            const initialProjectId = preselectedProjectId && projectOptions.includes(preselectedProjectId)
                ? preselectedProjectId
                : (selector.value || fallbackProjectId);
            selectProject(initialProjectId);
        } else if (projectsData && projectsData.length > 0) {
            selectProject(projectsData.find((project) => Array.isArray(project.phases) && project.phases.length > 0)?.id || projectsData[0].id);
        } else {
            renderEmptyState();
        }
    });
...
<?php echo $__env->make('layouts.client', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\dashboard\resources\views\client\timeline.blade.php ENDPATH**/ ?>