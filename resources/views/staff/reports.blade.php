            ...
            <div class="table-pagination-strip">
                ...
                <div id="reportsPagination" class="pagination-bar">
                    @if($reports->currentPage() > 1)
                        <button type="button" class="pagination-button" data-page="{{ $reports->currentPage() - 1 }}" aria-label="Previous page">â€¹</button>
                    @endif
                    ...
                    @if($reports->hasMorePages())
                        <button type="button" class="pagination-button" data-page="{{ $reports->currentPage() + 1 }}" aria-label="Next page">â€º</button>
                    @endif
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function renderPagination(pagination) {
                ...
                const buttons = [];
                buttons.push(`<button type="button" class="pagination-button" data-page="${Math.max(1, pagination.current_page - 1)}" ${pagination.current_page === 1 ? 'disabled' : ''} aria-label="Previous page">â€¹</button>`);
                ...
                buttons.push(`<button type="button" class="pagination-button" data-page="${Math.min(pagination.last_page, pagination.current_page + 1)}" ${pagination.current_page === pagination.last_page ? 'disabled' : ''} aria-label="Next page">â€º</button>`);
            ...
            function renderDetailsPanel(report) {
                ...
                if (adminImageUpload) {
                    ...
                    adminImageUpload.addEventListener('change', function () {
                        ...
                        if (uploadLabel) {
                            uploadLabel.textContent = `${files.length} file(s) selected â€” click to change`;
                        }
                    });
                }
                ...
                Swal.fire({
                    ...
                    fetch(`${detailsBaseUrl}/${reportId}/update`, {
                        ...
                        .then(payload => {
                            Swal.fire({
                                title: 'Changes Saved',
                                text: payload.message || 'Report updated successfully. Reloadingâ€¦',
                                icon: 'success',
                                confirmButtonColor: '#1c6b43',
                                timer: 1400,
