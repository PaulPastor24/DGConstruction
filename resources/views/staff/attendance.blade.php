...
<div class="attendance-page">
    ...
    @if(in_array(auth()->user()?->role, ['engineer', 'admin', 'administrator'], true))
        <section class="card border-0 shadow-sm rounded-4 mb-4">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div>
                        <h2 class="h5 mb-1">Role attendance schedules</h2>
                        <p class="text-muted small mb-0">Set time in, time out, and the allowed break for each role. OT starts after the saved time out.</p>
                    </div>
                </div>
                <div class="row g-3">
                    @foreach(['staff' => 'Staff', 'worker' => 'Worker'] as $roleKey => $roleLabel)
                        @php
                            $rule = $scheduleRules->firstWhere('role', $roleKey);
                            $defaults = \App\Models\AttendanceScheduleRule::defaultsForRole($roleKey);
                        @endphp
                        <div class="col-lg-6">
                            <form method="POST" action="{{ $rule ? route('admin.attendance.schedules.update', $rule) : route('admin.attendance.schedules.store') }}" class="border rounded-3 p-3 h-100">
                                @csrf
                                @if($rule) @method('PUT') @endif
                                <input type="hidden" name="role" value="{{ $roleKey }}">
                                <h3 class="h6">{{ $roleLabel }}</h3>
                                <div class="row g-2">
                                    <div class="col-6"><label class="form-label small">Time in</label><input class="form-control" type="time" name="start_time" value="{{ substr($rule->start_time ?? $defaults['start_time'], 0, 5) }}" required></div>
                                    <div class="col-6"><label class="form-label small">Time out / OT start</label><input class="form-control" type="time" name="end_time" value="{{ substr($rule->end_time ?? $defaults['end_time'], 0, 5) }}" required></div>
                                    <div class="col-6"><label class="form-label small">Break start</label><input class="form-control" type="time" name="break_start_time" value="{{ !empty($rule?->break_start_time) ? substr($rule->break_start_time, 0, 5) : substr($defaults['break_start_time'], 0, 5) }}"></div>
                                    <div class="col-6"><label class="form-label small">Break end</label><input class="form-control" type="time" name="break_end_time" value="{{ !empty($rule?->break_end_time) ? substr($rule->break_end_time, 0, 5) : substr($defaults['break_end_time'], 0, 5) }}"></div>
                                </div>
                                <div class="d-flex gap-2 mt-3">
                                    <button class="btn btn-success btn-sm" type="submit">Save {{ $roleLabel }} schedule</button>
                                </div>
                            </form>
                            @if($rule)
                                <form method="POST" action="{{ route('admin.attendance.schedules.destroy', $rule) }}" class="mt-2" onsubmit="return confirm('Remove this custom schedule and restore the default?')">
                                    @csrf @method('DELETE')
                                    <button class="btn btn-outline-danger btn-sm" type="submit">Reset default</button>
                                </form>
                            @endif
                        </div>
                    @endforeach
                ...
                <tbody>
                    @forelse($logs as $log)
                        ...
                        >
                            ...
                            <td data-label="Admin Edit">
                                <details>
                                    <summary class="btn btn-sm btn-outline-primary">Edit times</summary>
                                    <form method="POST" action="{{ route('admin.attendance.update', $log) }}" class="mt-2 p-2 border rounded bg-light">
                                        @csrf
                                        @method('PUT')
                                        <label class="small">Time in<input class="form-control form-control-sm" type="time" name="time_in" value="{{ $log->time_in ? substr($log->time_in, 0, 5) : '' }}"></label>
                                        <label class="small mt-1">Break out<input class="form-control form-control-sm" type="time" name="break_out" value="{{ $log->break_out ? substr($log->break_out, 0, 5) : '' }}"></label>
                                        <label class="small mt-1">Break in<input class="form-control form-control-sm" type="time" name="break_in" value="{{ $log->break_in ? substr($log->break_in, 0, 5) : '' }}"></label>
                                        <label class="small mt-1">Time out<input class="form-control form-control-sm" type="time" name="time_out" value="{{ $log->time_out ? substr($log->time_out, 0, 5) : '' }}"></label>
                                        <label class="small mt-1">Status<select class="form-select form-select-sm" name="status"><option value="present" @selected($status === 'present')>Present</option><option value="late" @selected($status === 'late')>Late</option><option value="half_day" @selected(in_array($status, ['half_day', 'half day'], true))>Half day</option><option value="absent" @selected($status === 'absent')>Absent</option><option value="on_leave" @selected($status === 'on_leave')>On leave</option></select></label>
                                        <label class="small mt-1">Remarks<textarea class="form-control form-control-sm" name="remarks" rows="2">{{ $log->remarks }}</textarea></label>
                                        <button class="btn btn-primary btn-sm mt-2" type="submit">Save attendance</button>
                                    </form>
                                </details>
                            </td>
                        </tr>
