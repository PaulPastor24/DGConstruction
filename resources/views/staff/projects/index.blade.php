                        ...
                        <tbody id="projectArchivesTableBody">
                            @forelse ($archives ?? collect() as $archive)
                                ...
                                <tr class="archive-row"
                                    ...
                                    <td>{{ $archive->project_location ?: optional($archive->project)->location ?: optional($archive->project)->project_location ?: 'â€”' }}</td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="small text-muted">Start: {{ $archive->start_date ? $archive->start_date->format('M d, Y') : 'â€”' }}</div>
                                        <div class="small text-muted">Target: {{ $archive->target_end_date ? $archive->target_end_date->format('M d, Y') : 'â€”' }}</div>
                                        <div class="small text-muted">Actual: {{ $archive->actual_end_date ? $archive->actual_end_date->format('M d, Y') : 'â€”' }}</div>
                                    </td>
                                    <td>{{ $archive->archived_at ? $archive->archived_at->format('M d, Y H:i') : 'â€”' }}</td>
                                    <td>
                                        @if($archive->project)
                ...
                <form action="{{ old('project_id', session('edit_project_id')) ? route('admin.projects.update', ['project' => old('project_id', session('edit_project_id'))]) : route('admin.projects.index') }}" method="POST" id="editProjectForm" class="edit-project-form" enctype="multipart/form-data">
                    ...
                    <div class="modal-section-card">
                        ...
                        <div class="mt-3">
                            ...
                            <div class="form-text small text-muted mt-1" id="editStatusHelpText">Choose the projectâ€™s current lifecycle state. Completed projects cannot be moved back to planning or in progress.</div>
                            @error('status')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.renderArchivedProjectsPage = function (pageNumber) {
        ...
        if (summaryEl) {
            ...
            summaryEl.textContent = visibleRows.length ? 'Showing ' + start + 'â€“' + end + ' of ' + visibleRows.length + ' archived projects' : 'No archived projects match the current filters';
        }
    };
    ...
    function populateEditProjectModal(project, supervisorId, triggerBtn) {
        ...
        if (statusSelect) {
            ...
            if (helpText) {
                helpText.textContent = 'Choose the projectâ€™s current lifecycle state. Completed projects cannot be moved back to planning or in progress.';
            }
